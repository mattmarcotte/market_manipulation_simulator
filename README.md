...
- **Accelerated time**: one simulated trading day every 2.5 seconds, rendered as daily candles.
...
- Every post is analyzed by **Gemini** for market impact. Posts can move one stock, ripple across a sector, or do nothing (absurd/no-impact posts are ignored by the market).
...
| **Account Service** (`services/account-service`) | Node, @grpc/grpc-js | Cash & positions on :50051. `CheckBalance`, `CheckPosition`, `GetPortfolio`, `ApplyFill`. Deduplicates fills by order id across the gRPC and Kafka paths. |
